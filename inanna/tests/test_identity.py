...
class IdentityTests(unittest.TestCase):
    ...
    def test_permitted_tools_list_network_eye_tools(self) -> None:
        ...
        self.assertIn("search_packages", tools)
        self.assertIn("list_packages", tools)
        self.assertIn("install_package", tools)
        self.assertIn("remove_package", tools)
        self.assertIn("calendar_today", tools)
        self.assertIn("calendar_upcoming", tools)
        self.assertIn("calendar_read_ics", tools)
        self.assertIn("browser_read", tools)
        self.assertIn("browser_search", tools)
    ...
    def test_current_phase_names_the_constitutional_filter(self) -> None:
        self.assertIn("Constitutional Filter", CURRENT_PHASE)
    ...
    def test_cycle4_preview_names_civic_roles(self) -> None:
        self.assertIn("guardian", CYCLE4_PREVIEW.lower())
        self.assertIn("operator", CYCLE4_PREVIEW.lower())
        self.assertIn("user", CYCLE4_PREVIEW.lower())

    def test_cycle4_summary_describes_civic_layer(self) -> None:
